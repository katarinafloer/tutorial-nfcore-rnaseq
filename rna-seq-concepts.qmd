...
## From sample to counts: the basic steps

Here is the overall journey from a biological sample to a number you can analyze statistically.

1. **Extract RNA** from the tissue, this happens in the wet lab before any computing begins
2. **Sequence the RNA**, the sequencing machine converts RNA fragments into short text strings called **reads**, stored in FASTQ files
3. **Align the reads** to a reference genome, figure out which gene each read came from
4. **Count the reads per gene**, for each gene, count how many reads mapped to it across each sample
5. **Compare counts across conditions**, use statistics to find genes where the count is higher or lower in one group vs another (differential expression)

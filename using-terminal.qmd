...
Right now `notes.txt` is inside the `tutorial` folder. Move it up one level to the Desktop:

```bash
mv notes.txt ../
```

`mv` stands for **move**. The destination `../` means "one folder up" — in this case, the Desktop.
